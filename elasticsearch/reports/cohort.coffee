      ...
      aggs:
        value:
          terms:
            script: """
                firstLoginDate = -1
                for (i in _source.events)
                  if (i.name == "signin" && firstLoginDate == -1)
                    firstLoginDate = i
                if (firstLoginDate != -1)
                  firstLoginDate = Minutes.minutesBetween(doc['date'].date, new DateTime(firstLoginDate.date)).getMinutes()
